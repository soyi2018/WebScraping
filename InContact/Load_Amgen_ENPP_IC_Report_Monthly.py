    ...
    if len(os.listdir(inbound)) ==2:
        ...
        for name in os.listdir(inbound):
            if 'IC_Reports_AgentSummary'.upper() in name.upper():
                # print(name)
                cur = conn.cursor()
                cur.execute('truncate table [BI_TotalCare_ENPP].[dbo].[Amgen_ENPP_Contact_Center_IC_Report_Monthly_Summary_Stg]')
                cur.commit()
             ...
             #   print(df)
              ...
              #  print(df)
                df.to_sql('Amgen_ENPP_Contact_Center_IC_Report_Monthly_Summary_Stg',engine,schema='dbo',if_exists='append',index=False)  
                cur.execute("declare @EM varchar(4000); Exec dbo.usp_Load_Contact_Center_IC_Reports_Monthly 'IC_Report_Monthly_Summary', @Error = @EM OUTPUT; select @EM")
               # rows = cur.fetchall()
                #EM = rows[0][0]
                if EM =='No':
                    cur.commit()
            ...
            if 'IC_Reports_AgentUnavailableTime'.upper() in name.upper():
                # print(name)
                cur = conn.cursor()
                cur.execute('truncate table [BI_TotalCare_ENPP].[dbo].[Amgen_ENPP_Contact_Center_IC_Reports_AgentUnavailableTime_Monthly_Stg]')
                cur.commit()
              ...
              #  print(df)
                ...
                df.to_sql('Amgen_ENPP_Contact_Center_IC_Reports_AgentUnavailableTime_Monthly_Stg',engine,schema='dbo',if_exists='append',index=False)  
                cur.execute("declare @EM varchar(4000); Exec dbo.usp_Load_Contact_Center_IC_Reports_Monthly 'AgentUnavailableTime', @Error = @EM OUTPUT; select @EM")
                #rows = cur.fetchall()
                #EM = rows[0][0]
                if EM =='No':
                    cur.commit()
